...
# MAGIC %sql
# MAGIC create database if not exists bbart_test;
...
#file=['account','bbcrm_log__c']
#'bbdrc_branch_hierarchy','borrowing_option__c','cif_account_team_rel','cif_business_customer','cif_customer_account_rel','llc_bi__loan__c','llc_bi__product_package__c','llc_bi__product_package_team__c','llc_bi__role__c','llc_bi__covenant2__c','llc_bi__covenant_type__c','llc_bi__legal_entities']
#file=['llc_bi__fee__c','term_condition__c','term_condition_type__c','product2']
#file=['opportunitylineItem','opportunity']
#file=['llc_bi__collateral__c','llc_bi__account_collateral','llc_bi__loan_collateral2__c','llc_bi__connection__c','guarantee_in_support_of__c','llc_bi__account_covenant__c','llc_bi__accountdocument__c','llc_bi__collateral_type__c','llc_bi__covenant_compliance2__c','llc_bi__loan_covenant__c','llc_bi__policy_exception__c','llc_bi__policy_exception_template__c','llc_bi__product_feature__c','loan_term_condition__c','risk_rating_historical_data__c','sic_code_table__c','user','currencytype','llc_bi__connection_role__c','llc_bi__document_placeholder__c','llc_bi__llc_loandocument__c','llc_bi__excluded_exposure__c','lon_business_facility']
#file=['lon_business_account']
#file=['lon_loan_information']
#file=['llc_bi__covenant_compliance2__c', 'llc_bi__covenant_type__c']
file=['llc_bi__collateral__c']

# COMMAND ----------

for x in file:
    df = adido_in(f"ADIDO-4189-{x}", delimiter=',')
    df.write.format("delta").mode("overwrite").saveAsTable(f"bbart_test.{x}")

# COMMAND ----------

spark.sql(f"select * from bbart_test.account limit 10").display()